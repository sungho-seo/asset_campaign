...
// PRD §4.2 — 권고 안내 페이지.
// F-NOTICE-1,2,3,4,5,7 구현. 본문 텍스트는 i18n 분리 (F-NOTICE-9 권장).
...
export default function NoticePage() {
  ...
  // F-NOTICE-3: 체크박스 필수. 응답이 이미 있으면 변경 흐름이므로 기본 true 제안.
  const [acknowledged, setAcknowledged] = useState<boolean>(!!latest);
  const [ownership, setOwnership] = useState<NoticeOwnership | null>(
    latest?.ownership ?? null
  );

  const canSubmit = acknowledged && ownership !== null;

  const handleSubmit = () => {
    if (!canSubmit || !ownership) return;
  ...
  // i18n bodyParagraphs는 string[] 타입을 반환받는다.
  const bodyParagraphs = t('notice.bodyParagraphs', {
    returnObjects: true,
  }) as string[];

  return (
    <Shell className="max-w-[820px]">
      {/* 헤딩 */}
      <header className="mb-6">
        <div className="mb-1 font-mono text-[11px] font-semibold uppercase tracking-wider text-brand">
          {t('notice.eyebrow')}
        </div>
        <h1 className="flex items-center gap-2 text-2xl font-semibold tracking-tighter2 text-text">
      ...
      {/* 이전 응답 요약 (F-NOTICE-7 컨텍스트 — 재진입 시) */}
      {latest && (
      ...
      {/* 권고 본문 카드 */}
      <section className="relative mb-6 overflow-hidden rounded-lg border border-line bg-white shadow-sm">
        <span aria-hidden className="absolute inset-y-0 left-0 w-[3px] bg-brand" />
        <div className="space-y-3 px-6 py-5 text-[13px] leading-relaxed text-text-2">
      ...
      <section className="rounded-lg border border-line bg-white p-6 shadow-sm">
        ...
        <label
          className={cn(
            'flex cursor-pointer items-start gap-3 rounded-md border bg-white p-3.5 transition-colors',
            acknowledged
        ...
        >
          <input
            ...
            className="mt-[3px] h-4 w-4 flex-shrink-0 cursor-pointer accent-brand"
          />
          <div className="min-w-0 flex-1">
